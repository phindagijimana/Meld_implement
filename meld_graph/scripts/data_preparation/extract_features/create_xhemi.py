...
# Helper function to get the wrapper directory path (resolves symlinks for path consistency)
def get_wrapper_dir():
    """Get the absolute path to the .freesurfer_wrappers directory, resolving symlinks."""
    # Use realpath to resolve any symlinks and get the canonical path
    script_dir = os.path.dirname(os.path.realpath(__file__))
    # Navigate up 4 levels: extract_features -> data_preparation -> scripts -> meld_graph -> root
    root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(script_dir))))
    wrapper_dir = os.path.join(root_dir, ".freesurfer_wrappers")
    return wrapper_dir
...
def create_xhemi(subject_id, subjects_dir,template = 'fsaverage_sym', verbose=False):
    ...
    # Get wrapper directory path (resolves symlinks for consistency)
    wrapper_dir = get_wrapper_dir()
    surfreg_path = os.path.join(wrapper_dir, "surfreg")

    if not os.path.isfile(opj(subjects_dir,subject_id,'surf','lh.fsaverage_sym.sphere.reg')):
        command = f'export SUBJECTS_DIR={subjects_dir} && {surfreg_path} --s {subject_id} --t {template} --lh --no-annot'    
        proc = Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
        stdout, stderr= proc.communicate()
        if verbose:
            print(stdout)
        # Check if output file was created (surfreg may return non-zero due to FreeSurfer bug but still create the file)
        if not os.path.isfile(opj(subjects_dir,subject_id,'surf','lh.fsaverage_sym.sphere.reg')):
    ...
    if not os.path.isfile(opj(subjects_dir,subject_id,'xhemi','surf','lh.fsaverage_sym.sphere.reg')):
        command = f'export SUBJECTS_DIR={subjects_dir} && {surfreg_path} --s {subject_id} --t {template} --lh --xhemi --no-annot'
        proc = Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
        stdout, stderr= proc.communicate()
        if verbose:
            print(stdout)
