...
def sample_flair_smooth_features(subject_id, subjects_dir, verbose=False):
    ...
    if os.path.isfile(opj(subjects_dir, subject_id, "mri", "FLAIR.mgz")):
        ...
        for dsf in ds_features_to_generate:
            ...
            hemi = dsf[0]
            d = dsf[1]
            # Use regheader instead of trgsubject to avoid path doubling with containerized FreeSurfer
            command = f"{os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), '.freesurfer_wrappers', 'mri_vol2surf')} --src {subjects_dir}/{subject_id}/mri/FLAIR.mgz --out {subjects_dir}/{subject_id}/surf_meld/{hemi}.gm_FLAIR_{d}.mgh --hemi {hemi} --projfrac {d} --regheader {subject_id} --surf white"
        ...
        for dswmf in dswm_features_to_generate:
            ...
            dwm = dswmf[1]
            # Use regheader instead of trgsubject to avoid path doubling with containerized FreeSurfer
            command = f"{os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), '.freesurfer_wrappers', 'mri_vol2surf')} --src {subjects_dir}/{subject_id}/mri/FLAIR.mgz --out {subjects_dir}/{subject_id}/surf_meld/{hemi}.wm_FLAIR_{dwm}.mgh --hemi {hemi} --projdist -{dwm} --regheader {subject_id} --surf white"
            proc = Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
            stdout, stderr= proc.communicate()
            if verbose:
                print(stdout)
    ...
    for hemi in hemispheres:
        ...
        command = f"SUBJECTS_DIR={subjects_dir} {os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), '.freesurfer_wrappers', 'mris_curvature_stats')} -f white -g --writeCurvatureFiles {subject_id} {hemi} curv"
        proc = Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
        stdout, stderr= proc.communicate()
        if verbose:
            print(stdout)
        ...
        command = f"SUBJECTS_DIR={subjects_dir} {os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), '.freesurfer_wrappers', 'mris_curvature_stats')} -f pial -g --writeCurvatureFiles {subject_id} {hemi} curv"
        proc = Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
        stdout, stderr= proc.communicate()
        if verbose:
            print(stdout)
        ...
        command = f"SUBJECTS_DIR={subjects_dir} {os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), '.freesurfer_wrappers', 'mris_fwhm')} --s {subject_id} --hemi {hemi} --cortex --smooth-only --fwhm 20 --i {subjects_dir}/{subject_id}/surf_meld/{hemi}.pial.K_filtered.mgh --o {subjects_dir}/{subject_id}/surf_meld/{hemi}.pial.K_filtered.sm20.mgh"
